    ...
    <div className="max-w-[1200px] mx-auto px-4 py-6 flex gap-6">
      ...
      <aside className="hidden lg:block w-60 flex-shrink-0 space-y-4">
        <div className="bg-white border border-gray-200 rounded p-4">
          <h3 className="text-sm font-bold text-[#003a70] border-b border-gray-200 pb-2 mb-3">오늘의 게임</h3>
          <div className="space-y-2">
            {games.slice(0, 3).map((g, i) => (
              <div
                key={g.gameId}
                onClick={() => onSelectGame(g)}
                className="flex items-start gap-2 cursor-pointer hover:bg-blue-50 p-1 rounded"
              >
                <span className="text-[#003a70] font-bold text-sm flex-shrink-0">{i + 1}</span>
        ...
        <div className="bg-[#003a70] text-white rounded p-4 text-center">

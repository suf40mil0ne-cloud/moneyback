...
export default function RankingBoard({ rankings }: Props) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded p-4">
      <h3 className="text-sm font-bold text-[#003a70] border-b border-gray-200 pb-2 mb-3">
        순위표 (누적 점수)
      </h3>
      {rankings.length === 0 ? (
          ...
          {rankings.slice(0, 10).map((entry, i) => (
            <li
              ...
              className={`flex items-center gap-2 text-xs py-1 px-2 rounded ${
                entry.userId === user.userId ? 'bg-blue-50 border border-[#003a70]/20' : ''
              }`}
            >
              ...
              <span className="font-bold text-[#003a70]">{entry.totalPoints}pt</span>
            </li>
          ))}
        </ul>
      )}
